  ...
  <%
	...
	<section class="context-nei">
		...
		<div class="input-box">
			...
			<h1 class="input-box-title">Change Bankaccount Limit</h1>
			<p class="input-box-title-p">Change MeyBank Transfer Limit</p>
			...
			<p style="color:red;padding:20px 0px 0px 30px;font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;"><%= msg %></p>
			...
			<section class="section-box-acc">
				<div class="box-acc">
					...
					<ul>
						...
						<li>The function allows you to reduce the Current Third Party Transfer Limit or increase the limit to a maxmimum of RM20,000.00</li>
					</ul>
				</div>
			</section>
  		...
  		$.ajax({
				...
				success:function(data){
					...
					$.each(data,function(x,y){
						...
						$("#tbody").append(`
							<tr>
								<td><a class="senda" href='../../Changelimit?id=${"${y.BankAccountID}"}'>${"${y.BankAccNo}"}</a></td>
								<td>${"${y.AccountType}"}</td>
								<td>MYR</td>
								<td>${"${balance.toFixed(2)}"}</td>
								<td>${"${y.AllLimit.toFixed(2)}"}</td>
							</tr>
						`)
